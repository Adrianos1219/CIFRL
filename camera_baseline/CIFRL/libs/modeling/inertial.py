...
class Fourier(nn.Module):
    def __init__(self,
        ...
        if kwargs:
            print("Fourier.__init__ got unexpected arguments, which will be ignored: "
        ...
        self.args = torch.linspace(-(history_horizon) * self.dt / 2, (history_horizon) * self.dt / 2, self.history_horizon, dtype=torch.float, device=self.device)
