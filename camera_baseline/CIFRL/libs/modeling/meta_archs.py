...
from .causal import causal_process
from .inertial import Fourier
...
@register_meta_arch("CIFRL")
class CIFRL(nn.Module):
    """
    ...
    def __init__(
        ...
        self.causal = causal_process(2048, 2048)
        self.fft = Fourier(600, 2304, 50,device="cuda:0")
    ...
    def forward(self, video_list):
        ...
        video_data = batched_inputs[:,:2048,:]
        l, causal_data = self.causal(video_data)
        causal_data = causal_data.permute(0, 2, 1)
        inertial_data = batched_inputs[:,2048:,:]
        inertial, _, _, _ = self.fft(inertial_data)
        batched_inputs = torch.cat([causal_data, inertial.squeeze(0)], dim=1).to(batched_masks.device)
